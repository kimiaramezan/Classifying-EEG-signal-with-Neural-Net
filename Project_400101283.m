...
customTrainData = TrainData(:, 1001:end, :);

stat_feature_matrix = zeros(3200,550);
freq_feature_matrix = zeros(1000,550);
...
for i = 1:550
    counter = 1476;
    for j = 1:59
        stat_feature_matrix(j, i) = var(customTrainData(j, :, i));

        % Calculate AR coefficients for each channel (12 samples)
        ar_coeff = calculate_ar_coefficients(customTrainData(j, :, i));
        stat_feature_matrix(60 + (j - 1) * 12 : 71 + (j - 1) * 12, i) = ar_coeff;
        
        % Calculate amp histogram for each channel (10 samples)
        amp_hist = calculate_amp_histogram(customTrainData(j, :, i));
        stat_feature_matrix(768 + (j - 1) * 10 : 768 + j * 10 - 1, i) = amp_hist;

        % Calculate and store the form factor for each channel
        channel_data = customTrainData(j, :, i);  % Extract the channel data
        form_factor = calculateFormFactor(channel_data);
        stat_feature_matrix(1357 + j, i) = form_factor;

        % Calculate Skewness for each channel
        skewness_value = calculateSkewness(customTrainData(j, :, i));
        stat_feature_matrix(1416 + j, i) = skewness_value;

        % Calculate Covariance for each pair of channels
        for k = j+1:59
            channel1_data = customTrainData(j, :, i);
            channel2_data = customTrainData(k, :, i);
            covariance_value = calculateCovariance(channel1_data, channel2_data);
            stat_feature_matrix(counter, i) = covariance_value;
            counter = counter + 1;
        end
...
normalized_stat_feature_matrix = zeros(size(stat_feature_matrix));
normalized_freq_feature_matrix = zeros(size(freq_feature_matrix));
normalized_stat_feature_matrix = myCustomScaler(stat_feature_matrix, [variance_range, ar_coeff_range, amp_histogram_range, form_factor_range, skewness_range, covariance_range]);
normalized_freq_feature_matrix = myCustomScaler(freq_feature_matrix, [max_freq_range, mid_freq_range, mean_freq_range, band_power_range]);

% Save the normalized matrices
save('normalized_stat_feature_matrix.mat', 'normalized_stat_feature_matrix');
save('normalized_freq_feature_matrix.mat', 'normalized_freq_feature_matrix');

function normalized = myCustomScaler(features, range)
    min_val = min(features(range, :), [], 1);
    max_val = max(features(range, :), [], 1);
    normalized = (features(range, :) - min_val) ./ (max_val - min_val);
end
