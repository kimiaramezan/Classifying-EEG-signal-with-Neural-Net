...
load('new_combined_matrix2.mat');
load('new_combined_matrix_Test2.mat');
...
for spread = spread_range
    for hidden_neurons = hidden_neurons_range
        ...
        for p = 1:num_folds
            ...
            train_data_fold = new_combined_matrix2(:, train_indices);
            train_labels_fold = TrainLabels(train_indices);
            
            valid_data_fold = new_combined_matrix2(:, valid_indices);
...
netrbf = newrb(new_combined_matrix2, TrainLabels, 0, spread, hidden_neurons);
train_predict = sim(netrbf, new_combined_matrix2);
...
for N = 34:50
    ...
    for p = 1:num_folds
        ...
        train_data = new_combined_matrix2(:, train_indices);
        train_labels = TrainLabels(1, train_indices);
        valid_data = new_combined_matrix2(:, valid_indices);
...
while epochs < max_epochs
    ...
    netmlp = train(netmlp, new_combined_matrix2, TrainLabels);

    train_predict = netmlp(new_combined_matrix2);
    train_predict = sign(train_predict);
    diff = train_predict - TrainLabels;
    ACC = 100 - sum(abs(diff)) / 11;
...
%%
% Test MLP
test_predict_mlp = netmlp(new_combined_matrix_Test2);
test_predict_label_mlp = sign(test_predict_mlp);

% Test RBF
test_predict_rbf = sim(netrbf, new_combined_matrix_Test2);
test_predict_label_rbf = sign(test_predict_rbf);

% Concatenate MLP and RBF test predictions vertically
test_predict_labels2 = [test_predict_label_mlp; test_predict_label_rbf];

% Save MLP and RBF test predictions
save('test_predict_labels2.mat', 'test_predict_labels2');

sum(abs(mean(test_predict_labels2)))
